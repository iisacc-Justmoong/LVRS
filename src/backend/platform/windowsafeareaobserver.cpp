...
SafeAreaSnapshot querySafeAreaSnapshot(QWindow *window)
{
    ...
    if (QNativeInterface::QAndroidApplication::isActivityContext()) {
        ...
        if (insets.isValid()) {
            ...
            if (QJniObject::getStaticField<jint>("android/os/Build$VERSION", "SDK_INT") >= 31) {
                // Android reports window-relative physical pixels. A null corner
                // means this window has no rounded corner at that position.
                snapshot.bottomCornerRadius = 0.0;
                for (const char *position : {"POSITION_BOTTOM_LEFT", "POSITION_BOTTOM_RIGHT"}) {
                    const jint index = QJniObject::getStaticField<jint>("android/view/RoundedCorner", position);
                    const auto corner = insets.callObjectMethod("getRoundedCorner", "(I)Landroid/view/RoundedCorner;", index);
                    if (corner.isValid())
                        snapshot.bottomCornerRadius = qMax(snapshot.bottomCornerRadius,
                            corner.callMethod<jint>("getRadius") / scale);
                }
            }
...
qreal WindowSafeAreaObserver::bottomCornerRadius() const
{
    return m_bottomCornerRadius;
}
...
void WindowSafeAreaObserver::updateMargins()
{
    ...
    const bool marginsDidChange = snapshot.left != m_leftInset
        ...
        || snapshot.bottomCornerRadius != m_bottomCornerRadius;
    ...
    m_bottomCornerRadius = snapshot.bottomCornerRadius;
