...
fn has_sentinels(path: &Path) -> bool {
    path.join("CMakeLists.txt").exists()
        && path.join("src/qml").is_dir()
        && path.join("src/backend").is_dir()
}
...
fn resolve_project_root_with_env_candidate(
    ...
    bail!(
        "failed to locate LVRS repository root from {} (expected CMakeLists.txt, src/qml, src/backend). Set {} to repository root when launching outside the tree.",
        cwd.display(),
        ENV_LVRS_ROOT
    )
}
...
fn validate_project_root_candidate_inner(
    ...
    bail!(
        "invalid LVRS project root from {}: {} (expected CMakeLists.txt, src/qml, src/backend)",
        source,
        normalized.display()
    )
}
...
fn install_source_snapshot(
        ...
        .with_context(|| format!("failed to read project root: {}", project_root.display()))?
    {
        ...
        if path.is_dir() && name == "src" {
            copy_source_snapshot_directory(&path, &source_install_dir.join(&name))?;
        } else if path.is_dir() {
...
fn copy_source_snapshot_directory(source: &Path, destination: &Path) -> Result<()> {
        ...
        .with_context(|| format!("failed to read source directory: {}", source.display()))?
    {
        ...
        if path.is_dir() {
            copy_source_snapshot_directory(&path, &destination.join(&name))?;
        } else {
...
mod tests {
    ...
    fn create_project_root(path: &Path) -> Result<()> {
        fs::create_dir_all(path.join("src/qml"))?;
        fs::create_dir_all(path.join("src/backend"))?;
        fs::write(
    ...
    fn resolve_project_root_prefers_checkout_over_env_install_snapshot() -> Result<()> {
        ...
        {
            let _cwd = CurrentDirGuard::enter(&checkout.join("src/qml"))?;
    ...
    fn install_source_snapshot_excludes_nested_build_artifacts() -> Result<()> {
        ...
        let rust_source = checkout.join("src/rust-cli").join("src");
        let rust_target = checkout.join("src/rust-cli").join("target").join("debug");
        let rust_build = checkout.join("src/rust-cli").join("build").join("debug");
        ...
        assert!(snapshot.join("src/rust-cli/src/main.rs").is_file());
        assert!(!snapshot.join("src/rust-cli/target").exists());
        assert!(!snapshot.join("src/rust-cli/build").exists());
