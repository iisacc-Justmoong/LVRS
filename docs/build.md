...
`lvrs_configure_qml_app()` sets `QT_QML_IMPORT_PATH` for installed-package consumption, applies a default executable output directory (`<build>/bin`) when unset, auto-links/imports LVRS static QML plugin artifacts for static package builds, and on Linux stages `lvrs-runtime/` beside the executable with the LVRS shared library plus QML module. `runBootstrappedQmlApp()` automatically probes Linux runtime QML import locations such as `lvrs-runtime/qml`, installed `../lib/qt6/qml`, and snapshot platform layouts before loading QML roots. `QmlAppLaunchSpec::initialProperties` is applied through `QQmlApplicationEngine::setInitialProperties(...)` immediately before the legacy single-root `loadFromModule(...)` path, and `QmlAppLaunchSpec::roots` can declare multiple `QmlRootLoadSpec` entries with per-root initial properties. Window roots can inherit an app-level `QmlWindowActivationPolicy` or use their own policy for `show()`, `raise()`, and `requestActivate()`. `QmlAppLaunchSpec::lifecycle` provides `after-root-loaded`, `after-window-activated`, and zero-delay `after-first-idle` hooks/queue tasks so low-priority domain startup can move out of handwritten `QTimer::singleShot(0, ...)` blocks. `ForegroundServiceGate` can guard one-time foreground service startup until a visible root window exists. `PermissionRequestSequencer` can run app-defined permission request steps sequentially and preserve in-memory request history. `BootstrapParallel` can run independent startup domain loads on a bounded worker pool and apply collected results on a selected QObject thread. `QmlContextBinder` can be used before root loading to expose C++ services as context properties and register C++ `ViewModel` subclasses into the `LV.ViewModels` singleton. `QmlTypeRegistrar` can register downstream app QML types from a manifest and return structured diagnostics for duplicate or failed registrations. Qt runtime deployment itself remains target-environment specific.
